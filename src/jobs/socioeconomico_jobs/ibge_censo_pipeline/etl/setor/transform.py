...
def _calcular_todos_indicadores(datasets: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    logger.info("Merge concluído: %d setores, %d colunas", len(df), len(df.columns))
    return df
...
def run(storage: Optional[StorageBackend] = None) -> TransformResult:
    ...
    pop_total = df["total_populacao"].sum()
    logger.info("=" * 60)
    logger.info("TRANSFORM CONCLUÍDO — %d setores | pop: %s | avisos: %d",
                len(df), f"{pop_total:,.0f}", len(avisos))
    logger.info("=" * 60)

    return TransformResult(df=df, setores=len(df), avisos=avisos, pop_total=pop_total)
