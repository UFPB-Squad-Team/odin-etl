...
Upsert dos indicadores socioeconômicos na coleção 'setor_indicadores',
agrupados sob o campo 'socioeconomico', com geometria do GPKG.

Estrutura do documento:
    {
        cd_setor: "250750705000001",   # chave única (15 dígitos)
        cd_municipio: "2507507",
        nm_municipio: "João Pessoa",
        uf: "PB",
        geometria: { type: "Polygon", ... },  # 2dsphere
...
def _carregar_geometrias() -> Dict[str, dict]:
    """Carrega geometrias do GPKG e retorna {cd_setor: geojson_dict}."""
    if not _GPKG_PATH.exists():
        logger.warning("GPKG não encontrado: %s — setores sem geometria.", _GPKG_PATH)
        return {}
...
def run(df: Optional[pd.DataFrame] = None, storage=None) -> int:
    """
    Upsert dos indicadores socioeconômicos de setor no MongoDB.

    Chave de upsert: cd_setor (string de 15 dígitos)
    ...
    if df is None:
        ...
        if not _GOLD_PATH.exists():
            raise FileNotFoundError(
                ...
                "Execute o transform primeiro: make run-socioeconomico-transform-setor"
            )
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            operacoes.append(UpdateOne({"cd_setor": cd_setor}, update, upsert=True))

        if sem_geom:
            logger.warning("%d setor(es) sem geometria no GPKG.", sem_geom)
