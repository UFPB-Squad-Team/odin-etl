...
from src.jobs._01_education.geocode_pipeline import extract, transform, load
...
def _get_geocode_fn():
    ...
    Currently uses Google Maps via geopy. To switch to Nominatim or
    another provider, replace this function — transform.py stays unchanged.
    """
    load_dotenv()
    ...
    if not api_key:
        raise ValueError(
            ...
            "Check your .env file. If you don't have a key yet, "
            "see docs/pipelines/pipelinesGeocoding/comparacao_analitica_api.MD "
            "for alternatives."
        )
